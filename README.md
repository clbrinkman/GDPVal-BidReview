...
- **上传入口** `/upload`：上传真实招标文件（平台的唯一原料；投标标书不上传，由 AI 生成基线）。
  docx 自动抽正文、zip 自动解包登记、PDF 配手贴公告文本；入 TDR-UPLD 编号，规则自动分类
...
- **扫描件处理（双路径）**：合成路径——扫描件由我们构造（参数化做旧），内容完全已知，
  GT 直接给到页码，证书有效期/编号/公章/签字做成显式校验项落在 N2 与专家区，
  真实度由"扫描件不真实"反馈通道收敛；真实路径——上传的扫描件先过本地 OCR（RapidOCR，无需 key）
  出文本层再解析，低置信字段（日期/金额/编号）进人工核对、按页直达阅读器
...
| 前缀 | 格式 | 实体 |
|---|---|---|
| `TDR` | `TDR-CCGP-{YYYY}-{NNNNN}` / `TDR-UPLD-{YYYY}-{NNNNN}` | tender 招标项目（爬虫 / 上传，按年递增） |
